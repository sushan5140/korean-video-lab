...
async function consumeAiBudget(token, action) {
  if(!token || !['micro','profile','creator','caption'].includes(action)) return false;
  try{return await rpc(token,'haneul_consume_ai_budget',{p_action:action})}
  catch{return false}
}
async function verifySignedIn(req){
 const token=bearer(req);
 if(!token)return {ok:false,status:401,error:'Sign in with Google to use transcripts.'};
 try{
  ...
  return user?.id?{ok:true,userId:user.id,token}:{ok:false,status:401,error:'Sign in with Google.'};
 }catch{return {ok:false,status:503,error:'Could not verify your session.'}}
}
...
module.exports={bearer,requestBytes,consumeAiBudget,verifyProviderAccess,verifySignedIn};
