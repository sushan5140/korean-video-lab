...
## Security hardening — 2026-09-25 (existing production app)

Project: `sushan5140/korean-video-lab`, existing Vercel `korean-video-lab`, Supabase `Haneul` only. No UI redesign, project replacement, database merge, or deletion.

- Existing Haneul `profiles`, per-learner records and admin/referral/AI grant tables have RLS. Admin-only creator and referral operations use authenticated SECURITY DEFINER RPCs; direct public table access is not the authorization mechanism.
- Newly added `lib/haneul-security.js` authenticates bearer tokens with Supabase and verifies `haneul_has_ai_access` before paid semantic/Groq enhancement. Without access, micro lessons retain the original transcript-derived deterministic fallback. Public API calls cannot trigger this paid Groq path merely by knowing an endpoint.
- New `haneul_ai_request_budgets` is RLS-protected and has no anon/authenticated direct table grants. `haneul_consume_ai_budget` atomically limits per signed-in user per rolling ten-minute bucket: micro 12, profile analysis 6, creator AI 12, paid captions 80. It has no caller-chosen limit argument. Existing progress and referral records were not changed.
- Grok profile analysis and creator AI retain existing signed-in/referral/admin checks and now have durable provider budgets; creator AI and translator endpoints have bounded input and upstream timeouts. Translation falls back to paid Groq only with verified AI access/budget. Free Google translation attempts still work for Korean text.
- Paid FreeTranscriptAPI captions now require a verified Google session and per-user caption budget. Without it the auto captions route can still use YouTube's public Korean track, while an explicit paid-provider request returns 401/429. The root lesson app, creator studio, ambassador studio and quality proxy pass their sessions to preserve the paid-caption path. Public YouTube captions are not described as private.
- Owner YouTube discovery configuration status now requires admin authentication before revealing configured state. The protected /internal collaborator UI still relies on its existing server-side/RLS checks for data, not DOM hiding.
- `vercel.json` adds HTTPS, MIME, clickjacking/frame-ancestor, no-index on internal admin route, referrer and basic feature restrictions; microphone remains allowed for speaking practice. Because the existing app uses inline HTML/CSS/JS, this is a **limited** CSP (frame-ancestors/object-src/base-uri), not a claim of strict script nonce-based CSP.
