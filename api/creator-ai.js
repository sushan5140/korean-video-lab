...
async function auth(req,code){
 ...
 const me=await fetch(SUPABASE+'/auth/v1/user',{headers});
 if(!me.ok)throw Error('Google session expired');
 const ok=await fetch(SUPABASE+'/rest/v1/rpc/haneul_creator_member',{method:'POST',headers,body:JSON.stringify({p_code:code})});
 if(!ok.ok||await ok.json()!==true)throw Error('Join this creator community to unlock its AI features');
 const ai=await fetch(SUPABASE+'/rest/v1/rpc/haneul_has_ai_access',{method:'POST',headers,body:'{}'});
...
module.exports=async function(req,res){
 ...
 try{
  ...
  const r=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{authorization:'Bearer '+process.env.GROQ_API_KEY,'content-type':'application/json'},body:JSON.stringify(body)});
